...
function SkillIcon({ label, bg, fg = "#ffffff" }) {
  return (
    <div
      className="w-14 h-14 rounded-2xl flex items-center justify-center font-display font-extrabold text-sm shrink-0"
...
export default function Skills() {
  ...
  return (
    <section id="skills" className="py-section-gap relative">
      <div className="container max-w-container-max mx-auto px-gutter relative z-10">
        <div className="text-center mb-16 animate-reveal stagger-1">
          <p className="font-label-md text-label-md text-primary uppercase tracking-[0.3em] mb-4">
            My Expertise
          </p>
          ...
          <p className="font-body-lg text-body-lg text-on-surface-variant max-w-2xl mx-auto">
            The languages, frameworks, and tools I've used building real, working projects.
          </p>
        </div>

        {/* Category tabs */}
        <div className="flex flex-wrap justify-center gap-4 mb-16 animate-reveal stagger-2">
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 px-6 py-3.5 rounded-2xl border font-label-md text-label-md font-bold uppercase tracking-widest transition-all ${
                  isActive
              ...
              >
                ...
                <span
                  className={`text-caption font-caption ${
                    isActive ? "text-on-primary/70" : "text-on-surface-variant/60"
                  }`}
                >
                  ({cat.skills.length})
                </span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 animate-reveal stagger-3">
          {active.skills.map((skill) => (
            <div
              key={skill.name}
              className="p-8 rounded-3xl bg-white/5 border border-white/10 flex flex-col items-center gap-4 transition-all hover:border-primary/40 hover:bg-primary/10 hover:backdrop-blur-xl hover:scale-105"
            >
              <SkillIcon label={skill.badge} bg={skill.bg} fg={skill.fg} />
              <span className="font-label-md text-label-md uppercase tracking-widest font-bold text-center">
                {skill.name}
              </span>
            </div>
          ))}
